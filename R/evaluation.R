...
estimateProb <- function(seu, ider, n_size = 40, seeds = c(12345, 89465, 10385, 10385, 17396), 
  ...
  pca <- seu@reductions$pca@cell.embeddings[idx, 1:15] # use first 15 PCs for spectral clustering
  
  for(itor in 1:n.perm) { # force the positive control group into first groups
    set.seed(seeds[itor])
    ...
    combinations <- combinations[!combinations$g1 %in% c("bg_Batch1", "bg_Batch2")  & 
