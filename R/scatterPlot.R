...
#' @param reduction Character. The dimension reduction used to plot. Common options: "pca", "tsne", "umap". The availability of dimension reduction can be checked by `Reductions(seu)`.
#' @param colour.by Character. One of the column names of `seu@meta.data`. Can be either discreet or continuous variables.
#' @param colvec A vector of Hex colour codes. If no value is given (default), a vector of 74 colours will be used.
#' @param title Character. Title of the figure.
#' @param sort.by.numbers Boolean. Whether to sort the groups by the number of cells. (Default: True)
...
scatterPlot <- function(seu, reduction, colour.by, colvec = NULL, 
                        title = NULL, sort.by.numbers = TRUE, viridis_option = "B") { # function for tSNE plot
  if(is.null(colvec)){
    colvec <- c('#7FC97F','#BEAED4','#FDC086','#FFFF99','#386CB0','#F0027F','#BF5B17','#666666','#1B9E77','#D95F02',
