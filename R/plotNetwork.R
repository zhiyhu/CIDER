...
plotNetwork <- function(seu, dist, col.vector = NULL, use = "coef", 
  ...
  if(is.null(col.vector)) {
    col.vector <- c('#7FC97F','#BEAED4','#FDC086','#FFFF99','#386CB0','#F0027F','#BF5B17','#666666','#1B9E77',
  ...
  if(length(unique(df_plot$g)) <= length(col.vector) & length(unique(df_plot$g)) > 1) {
    ...
    warning("Provided colours are less than needed. No colour is used.")
  }
  ...
  if(length(unique(df_plot$g)) > 1) {
    plot(net);legend(x=-1.5, y=-1.1, df_plot_cols$group, pch=21,
