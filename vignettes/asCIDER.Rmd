...
vignette: >
  %\VignetteIndexEntry{Run meta-clustering (asCIDER) on the pancreas dataset}
...
This vignette performs asCIDER on a cross-species pancreas dataset.
...
To exam if clustering will be confounded by the cross-species factors, we perform the conventional Seurat$^2$ clustering pipeline.
...
ider <- getIDEr(seu, 
                ...
                use.parallel = TRUE, verbose = FALSE)
```

```{r run-getIDER-no-parallel}
ider_noParallel <- getIDEr(seu, 
                group.by.var = "initial_cluster",
                batch.by.var = "Batch",
                downsampling.size = 35, 
                use.parallel = FALSE, verbose = FALSE)
identical(ider, ider_noParallel)
```
